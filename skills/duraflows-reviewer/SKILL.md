...
- [ ] **No writes to `ctx.metadata`.** Metadata is frozen. Writes are silently ignored. If you see `ctx.metadata.x = y`, it's a bug.
...
| `ctx.metadata.status = "done"`                                                                                         | Metadata is frozen, write is silently ignored                                                                    | Write to `ctx.context.status` instead                                                                         |
